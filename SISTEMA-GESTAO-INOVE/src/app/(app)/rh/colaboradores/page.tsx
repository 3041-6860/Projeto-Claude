...
export default function Colaboradores() {
  ...
  function admitir() {
    ...
    const novo: Colaborador = {
      nome: admNome, cargo: admCargo, dept: admDept, adm: admDataAdm,
      status: admContrato === 'Estágio' ? 'Estágio' : 'Ativo',
      ferias: false, ativo: true,
    }
    salvar([...lista, novo])
    setModalAdm(false)
    setAdmNome(''); setAdmCargo(''); setAdmDept(''); setAdmDataAdm('')
    setAdmEmail(''); setAdmCPF(''); setAdmNasc(''); setAdmSalario('')
    setAdmPerfil(''); setAdmBeneficios([])
  }
      ...
      {modalAdm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 640, maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,.25)' }}>
            ...
            <div style={{ background: RH_COLOR, padding: '18px 24px', borderRadius: '12px 12px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: RH_COLOR, paddingBottom: 6, borderBottom: '2px solid var(--border)' }}>Dados Pessoais</p>
              ...
              <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: RH_COLOR, paddingBottom: 6, borderBottom: '2px solid var(--border)', marginTop: 4 }}>Dados Profissionais</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: RH_COLOR, paddingBottom: 6, borderBottom: '2px solid var(--border)', marginTop: 4 }}>Benefícios</p>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {['Vale Transporte', 'Vale Refeição', 'Plano de Saúde', 'Plano Odontológico', 'Seguro de Vida', 'Gympass'].map(b => (
                  <label key={b} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, cursor: 'pointer', padding: '5px 10px', borderRadius: 6, border: `1px solid ${admBeneficios.includes(b) ? RH_COLOR : 'var(--border)'}`, background: admBeneficios.includes(b) ? '#e8f5e9' : '#fff' }}>
                    <input type="checkbox" checked={admBeneficios.includes(b)} onChange={() => toggleBeneficio(b)} style={{ accentColor: RH_COLOR }} />
                    {b}
                  </label>
                ))}
              </div>
          ...
          <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 560, maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,.25)' }}>
            ...
            <div style={{ padding: '14px 24px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 12, color: 'var(--gray)' }}>
                {dChecklist.length < checklistDeslig.length
                  ? `⚠️ ${checklistDeslig.length - dChecklist.length} itens do checklist pendentes`
                  : '✅ Checklist completo'}
              </span>
              <div style={{ display: 'flex', gap: 10 }}>
                <button type="button" onClick={() => setModalDeslig(null)} className="btn btn-outline">Cancelar</button>
                <button type="button" onClick={dispensar} style={{ background: '#c62828', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: 13 }}>
