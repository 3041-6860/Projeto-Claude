...
interface InitialUser { name: string; email: string; role: string }

export default function PerfilClient({ initialUser }: { initialUser: InitialUser }) {
  const email   = initialUser.email;
  const nome    = initialUser.name;
  ...
  useEffect(() => {
    try {
      const sp = localStorage.getItem(`inove-perfil-${email}`);
      if (sp) setPerfil(JSON.parse(sp));

      const ss = localStorage.getItem("inove-status-v1");
      if (ss) { const s = JSON.parse(ss)?.[email]; if (s) setStatus(s); }

      const sb = localStorage.getItem(`inove-banner-${email}`);
      if (sb) setBanner(sb);

      const st = localStorage.getItem("inove-tarefas-v2");
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (st) setTasks((JSON.parse(st) as any[]).filter(t => t.responsavel === nome || t.criador === nome));

      const sf = localStorage.getItem("inove-feed-posts-v2");
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (sf) setPosts((JSON.parse(sf) as any[]).filter(p => p.author === nome));

      const sk = localStorage.getItem("inove-ponto-v1");
      if (sk) {
        const store = JSON.parse(sk);
        const ur    = store[email] ?? {};
        const dias  = [];
        for (let i = 6; i >= 0; i--) {
          const d = new Date();
  ...
  return (
    <div style={{ maxWidth: 1080, margin: "0 auto" }}>
      ...
      <div style={{ borderRadius: 16, overflow: "hidden", border: "1px solid #e5e7eb", marginBottom: 0 }}>
        ...
        <div style={{ background: "white", padding: "0 24px 16px", position: "relative" }}>
          {/* Avatar */}
            ...
            <div style={{ position: "relative" }}>
              {perfil.foto ? (
                ...
                <div style={{
                  ...
                  fontSize: 34, fontWeight: 700, color: "white", userSelect: "none",
                }}>
                  {initials || "?"}
                </div>
              )}
          ...
          <div style={{ paddingTop: 16, paddingLeft: 128 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 5, flexWrap: "wrap" }}>
              ...
              {perfil.cargo && <span style={{ fontSize: 12, color: "#6b7280" }}>· {perfil.cargo}</span>}
              {perfil.departamento && <span style={{ fontSize: 12, color: "#6b7280" }}>· {perfil.departamento}</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ══ ABAS ══════════════════════════════════════════════ */}
      <div style={{ display: "flex", borderBottom: "1px solid #e5e7eb", background: "white", marginBottom: 24 }}>
        {TABS.map(t => {
          const badge =
          ...
          <div className="card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button type="button" onClick={salvar}
                style={{
                  padding: "7px 20px", background: saved ? "#059669" : avatarBg,
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px 20px" }}>
              ...
              <div style={{ gridColumn: "span 2" }}>
                ...
                <textarea
                  ...
                  rows={3}
                  style={{ width: "100%", padding: "7px 10px", border: "1px solid #e5e7eb", borderRadius: 7, fontSize: 13, resize: "vertical" }}
                />
              </div>
            </div>
          </div>
        ...
        <div className="card">
          ...
          {tasks.length === 0 ? (
            ...
            <table className="tbl">
              <thead>
                <tr>
                  <th>Tarefa</th><th>Status</th><th>Prioridade</th>
                  <th>Prazo</th><th>Papel</th><th>Vínculo</th>
                </tr>
              </thead>
              <tbody>
                {tasks.map(t => {
                  ...
                  return (
                    <tr key={t.id}>
                      ...
                      <td><span className="badge" style={{ background: avatarBg, color: "#fff" }}>{t.status}</span></td>
                      <td>
                      ...
                      <td style={{ fontSize: 12, color: "#6b7280" }}>
                        {t.vinculoNome ? `${t.vinculoTipo === "lead" ? "🙋" : "💼"} ${t.vinculoNome}` : "—"}
                      </td>
                    </tr>
                  );
                })}
        ...
        <div className="card">
          ...
          {posts.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {posts.map(p => (
                <div key={p.id} style={{ padding: "14px 16px", borderRadius: 10, border: "1px solid #e5e7eb", background: "#fafafa" }}>
                  ...
                  {p.eventTitle && <p style={{ fontWeight: 600, fontSize: 13, margin: "0 0 4px" }}>{p.eventTitle}</p>}
                  {p.fileName  && <p style={{ fontSize: 13, color: "#374151", margin: "0 0 4px" }}>📎 {p.fileName}</p>}
                  {p.content   && <p style={{ fontSize: 13, color: "#374151", margin: 0 }}>{p.content}</p>}
                </div>
              ))}
            </div>
          )}
