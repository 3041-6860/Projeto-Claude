...
import { listarHonorarios, salvarHonorario, removerHonorario, registrarPagamento } from "@/lib/financeiro-storage";
import type { Honorario, StatusHonorario, TipoHonorario } from "@/lib/financeiro-storage";
                ...
                return (
                  <tr key={h.id} style={{ borderTop: i > 0 ? "1px solid var(--border-light)" : undefined }}>
                    <td className="px-4 py-3">
                      ...
                      {h.processoNumero && (
                        <Link href={`/processos/${h.processoId}`} className="text-[10px] font-mono hover:underline" style={{ color: "var(--gcj-red)" }}>
                          {h.processoNumero}
                        </Link>
                      )}
                    </td>
